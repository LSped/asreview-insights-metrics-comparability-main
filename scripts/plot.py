...
def plot_cm(ax,
                ...
                x_screened=True,                
                x_absolute=False):
    """Plot the TP,FP,TN,FN@T for all thresholds T.
...
def _plot_cm(ax, labels, x_screened=True, x_absolute=False):
    ...
    y1 = _tp_values(labels, x_screened=x_screened)
    y2 = _fp_values(labels, x_screened=x_screened)
    y3 = _tn_values(labels, x_screened=x_screened)
    y4 = _fn_values(labels, x_screened=x_screened)
    ...
    # Find the index where TP curve crosses the threshold value
    index = np.where(np.array(y1) >= threshold)[0][0]


    # Add a vertical line at 100% recall
    ax.axvline(x[index], color='lightgray', linestyle='--', label='100% Rel recs')
